...
LoadScAtac <- function(sampleid = "control1",
 ...
 "Dnaja1", "Dnaja2", "Dnajb1", "Dnajb4",
 "Hspb1", "Hspb2", "Hspb6", "Hspb8",
 "Hspe1", "Hspd1"
)
 # Calculate feature percentages
 pbmc[["percent.mt"]] <- PercentageFeatureSet(pbmc, pattern = "^mt-")
 pbmc[["percent.rb"]] <- PercentageFeatureSet(pbmc, pattern = "^Rp[sl]")
 pbmc[["percent.hsp"]] <- PercentageFeatureSet(pbmc, features = hsp_genes)
...
for (i in 1:length(sample_name)) {
 ...
 file = file.path("./cellranger_outs/", sample_name[i], "/outs/atac_peaks.bed"),
...
combined.peaks <- reduce(x = c(peak.list[[1]],peak.list[[2]],peak.list[[3]],
 ...
 peak.list[[13]],peak.list[[14]],peak.list[[15]]))
...
sc_list <- lapply(sample_name, function(sample_id) {
 LoadScAtac(
 sampleid = sample_id,
 data_dir = "./cellranger_outs/",
 group_prefixes = c("NP", "EP", "MP", "LP", "PP"),
 group_names = c("NP", "EP", "MP", "LP", "PP"),
 annotation = annotation,
 combined.peaks = combined.peaks
 )
})
...
isOutlier <- function(qcidx,
 ...
 # Append indices of missing values and ensure uniqueness
 out <- base::unique(c(out, which(is.na(qcidx))))
 return(out)
}
...
for (i in 1:length(sample_name)) {
 ...
 x = outpt, y = Inf,
 label = paste("X =", outpt)
 ), # y=Infplace text as high as possible
 vjust = 1, # vertical text alignment;11 = near the top
 hjust = 0, # right-aligned
 inherit.aes = FALSE, # do not inherit defaultaesthetics
 size = 3
 ) +
 theme_bw()
...
saveRDS(sc_list,'data/sc_list_bfqc.Rds')
##
head(sc_list[[1]]@meta.data)
sc_list <- lapply(sc_list, function(x) {
 ...
 out.cell <- base::unique(c(
 isOutlier(x$log10nCount_RNA, type = "both",threshold = 5),
 isOutlier(x$log10nFeature_RNA, type = "both",threshold = 5),
 isOutlier(x$nucleosome_signal, type = "both",threshold = 5),
 isOutlier(x$TSS.enrichment, type = "both",threshold = 5),
...
for(i in 1:15){ 
 ATAC_list[[i]]@assays$ATAC@fragments[[1]]@path <- paste(
 "data/",
 sample_name[i],
...
saveRDS(integrated,'data/ATAC.Rds')
####################
...
mHeart <- RunUMAP(mHeart,dims = 2:30, reduction = "rlsi", reduction.name = "ATAC.umap")
######################
m.cc.genes <- readRDS("data/mouse_cell_cycle_genes.rds") 
