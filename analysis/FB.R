...
FB <-  readRDS("E:/2.工作/YF/mHeart/FB.Rds")
obj <- subset(FB,subtype %in% c('FB-1'))
#########miloR######
library(miloR)
...
library(patchwork)
DefaultAssay(FB) <- 'RNA'
FB@assays <- FB@assays['RNA']
traj_milo_sce <- as.SingleCellExperiment(FB)
...
#plot
distGenes(out, cluster = "FB-2") # 指定4这一群细胞
df <- data.frame(value = out$vals[["FB-2"]]$beta.hat, 
...
writexl::write_xlsx(res,'FigS9i_FB1_四组GO通路富集结果.xlsx')
...
samplegroup <- c('NP','MP','LP','PP')
#FB
FB <- RunDEtest(FB, group_by = "subtype", only.pos = FALSE, fc.threshold = 1,min.pct = 0.25)
...
enrich <- subset(res,Description %in% c('extracellular matrix organization','extracellular structure organization','AIM2 inflammasome complex assembly',
                                        'actin filament organization','response to transforming growth factor beta','heart valve morphogenesis',
                                        'cell-substrate adhesion','ameboidal-type cell migration','wound healing','collagen fibril organization',
                                        'extracellular matrix organization','extracellular structure organization','developmental cell growth',
                                        'muscle cell development','relaxation of muscle','action potential'))
...
ggplot(data = dt, aes(x = -log10(pvalue), y = rev(place), fill = Cluster)) +
  ...
  labs(x = "-Log10(pvalue)", y = "FB-5    FB-4      FB-3      FB-2    FB-1", 
...
###########promoter ATAC######
library(ggrastr)
setwd("E:/2.工作/YF/mHeart/8.终稿调整/20250315/figS9_FB")
...
for(j in c('MP','LP','PP')){
  ...
  p <- ggplot(avdata_df , aes(x = log10(NP), y = log10(test))) +
    ...
    labs(x = "log10(gene activity) of NP",
         ...
         title = 'FB')
  ggsave(paste('k_FB1_NPvs',j,'所有细胞染色质开放片段数散点图.pdf',sep = ''),p,
...
ggplot(ratiodt, aes(x = Column, y = Number, fill = Column)) +
  ...
  scale_y_continuous(expand = c(0, 0),limits = c(0, 4000))  # 标题居中
...
library(dplyr)
library(patchwork)
figR.d <-  readRDS("E:/2.工作/YF/mHeart/FB1_MPfigR.d.Rds")
...
# 将矩阵转换为图对象
graph <- as_tbl_graph(tf_target_matrix, dirFBted = TRUE)
# 添加节点属性：表达量和百分率
...
figR.d <-  readRDS("E:/2.工作/YF/mHeart/FB1_figR.d.Rds")
DefaultAssay(obj) <- 'motif'
ranklist <- rankDrivers(figR.d,rankBy = "meanScore")$data
rownames(ranklist) <- ranklist$Motif
#'extracellular matrix assembly' 'collagen biosynthetic process'
dt <- data.frame(t(GetAssayData(obj[ranklist$Motif,],assay = 'motif',slot = 'data')),
                 cardiac = GetAssayData(obj,assay = 'aUCell',slot = 'data')['collagen biosynthetic process',])
...
plot.dt$color <- ifelse(plot.dt$genes %in% c('Klf4','Klf2','Klf3'),'Klf','Others')
esrra_data <- subset(plot.dt, genes %in% c('Klf4','Klf2','Klf3'))
...
FeatureDimPlot(
  srt = subset(FB)
  , features = c('Zbtb18'), assay="activities",slot='data',bg_color = 'gray95',split.by = 'group',
  reduction = "umap", theme_use = "theme_blank",raster = F,ncol = 4,
)
p1 <- CoveragePlot(
  assay = 'ATAC',
  object = obj,
...
writexl::write_xlsx(DEGs_group,'FB_DEMotif.xlsx')
#gene
...
writexl::write_xlsx(DEGs_group,'FB1_DERNA.xlsx')

#########rougue###
library(ROGUE)
library(tibble)
expr <- data.frame(GetAssayData(FB,assay = "RNA",slot = "data"))
rogue.res <- rogue(expr, labels = FB$subtype, samples = FB$orig.ident, platform = "UMI",
                   span = 0.6)
...
for(j in c('MP','LP','PP')){
  ...
  p <- ggplot(avdata_df ) +
    ...
    labs(x = "log10(gene activity) of NP",
         y = paste("log10(gene activity) of ",j,sep = ''),
         title = 'FB1')
  ggsave(paste('FB1_NPvs',j,'所有细胞染色质开放片段数散点图.pdf',sep = ''),p,
         width = 3.84,height = 3.68)
}
...
DEG_nb <- data.frame()
  ...
  ggsave(paste('FB-1_组间DEG数.pdf',sep = ''),p,
         width = 3.3,height = 3)
...
ggsave(paste('FB-1_ATAC差异peak数.pdf',sep = ''),p,
       width = 3.3,height = 3)
...
act.dt <- AggregateExpression(FB, assays = 'activities',group.by = 'subtype')[[1]]
